...
def main():
    ...
    for lr in [0.0005, 0.001, 0.005, 0.01]:
        for mini_batch_size in [8, 16, 32, 64, 128, 256, 512]:
            ...
            m1, train_epoch_losses1, min_epoch1, min_loss1, tpe = model_training(in_model1, device, lr, num_epochs, mini_batch_size, fashion_mnist_train_data, models_directory)


            # evaluate the trained model
            bm = f'{m1}_model_e{num_epochs}_mb{mini_batch_size}_lr{lr}_epoch_{min_epoch1}.pth'
            eval_loss, mat, acc = model_evaluation(IaroHasNoFashionNet1(),'./models_fmnist/'+bm, fashion_mnist_eval_data, batch_size)
            m1_results.append([m1, num_epochs, lr, mini_batch_size, train_epoch_losses1, min_epoch1, min_loss1, tpe, eval_loss, mat, acc])

    results = pd.DataFrame(m1_results, columns=['model', 'num_epochs', 'lr', 'mini_batch_size', 'train_epoch_losses', 'min_epoch', 'min_loss', 'time_per_epoch', 'eval_loss', 'conf_matrix', 'accuracy'])
    results.to_pickle(models_directory + '/m1_training_results.pkl')
