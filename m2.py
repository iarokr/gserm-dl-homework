...
def main():
    ...
    class IaroHasNoFashionNet2(nn.Module):
        ...
        def __init__(self):
            ...
            self.linear1 = nn.Linear(8*8*20, 640, bias=True)  # the linearity W*x+b
            self.relu1 = nn.ReLU(inplace=True)  # the non-linearity

            # specify fc layer 2 - in 640, out 100
            self.linear2 = nn.Linear(640, 100, bias=True)  # the linearity W*x+b
    ...
    for lr in [0.0005, 0.001, 0.005, 0.01]:
        for mini_batch_size in [8, 16, 32, 64, 128, 256, 512]:
            ...
            m2, train_epoch_losses2, min_epoch2, min_loss2, tpe = model_training(in_model2, device, lr, num_epochs, mini_batch_size, fashion_mnist_train_data, models_directory)


            # evaluate the trained model
            bm = f'{m2}_model_e{num_epochs}_mb{mini_batch_size}_lr{lr}_epoch_{min_epoch2}.pth'
            eval_loss, mat, acc = model_evaluation(IaroHasNoFashionNet2(),'./models_fmnist/'+bm, fashion_mnist_eval_data, batch_size)
            m2_results.append([m2, num_epochs, lr, mini_batch_size, train_epoch_losses2, min_epoch2, min_loss2, tpe, eval_loss, mat, acc])

    results = pd.DataFrame(m2_results, columns=['model', 'num_epochs', 'lr', 'mini_batch_size', 'train_epoch_losses', 'min_epoch', 'min_loss', 'time_per_epoch', 'eval_loss', 'conf_matrix', 'accuracy'])
    results.to_pickle(models_directory + '/m2_training_results.pkl')
