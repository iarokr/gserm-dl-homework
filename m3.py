...
def main():
    ...
    for lr in [0.0005, 0.001, 0.005, 0.01]:
        for mini_batch_size in [8, 16, 32, 64, 128, 256, 512]:
            ...
            in_model3 = IaroHasNoFashionNet3()

            m3, train_epoch_losses3, min_epoch3, min_loss3, tpe = model_training(in_model3, device, lr, num_epochs, mini_batch_size, fashion_mnist_train_data, models_directory)


            # evaluate the trained model
            bm = f'{m3}_model_e{num_epochs}_mb{mini_batch_size}_lr{lr}_epoch_{min_epoch3}.pth'
            eval_loss, mat, acc = model_evaluation(IaroHasNoFashionNet3(),'./models_fmnist/'+bm, fashion_mnist_eval_data, batch_size)
            m3_results.append([m3, num_epochs, lr, mini_batch_size, train_epoch_losses3, min_epoch3, min_loss3, tpe, eval_loss, mat, acc])

    results = pd.DataFrame(m3_results, columns=['model', 'num_epochs', 'lr', 'mini_batch_size', 'train_epoch_losses', 'min_epoch', 'min_loss', 'time_per_epoch', 'eval_loss', 'conf_matrix', 'accuracy'])
    results.to_pickle(models_directory + '/m3_training_results.pkl')
